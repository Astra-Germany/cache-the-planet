...
- common private-key and high-confidence known-token patterns in text files up to 1 MiB, plus generic credential assignments in non-source/non-package-metadata text files. Token patterns require a realistic token length to avoid matching ordinary package text. Python package metadata such as `*.dist-info/METADATA`, `RECORD` and `WHEEL` is allowed because dependency names and descriptions can contain credential-related words. Known binary/archive formats such as JAR, ZIP, WAR and AAR are not decoded as text, preventing false positives from binary package data. Normal package source code is not rejected merely because it contains variables such as `password`, `secret` or `api_key`.
