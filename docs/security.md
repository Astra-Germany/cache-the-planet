...
- common private-key, token and credential string-value patterns in regular files up to 1 MiB. Type annotations and normal source expressions such as `password: str` or `password = None` are intentionally allowed to avoid rejecting package source code.
